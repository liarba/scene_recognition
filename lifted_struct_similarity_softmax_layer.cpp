//This code is from https://github.com/rksltnl/Deep-Metric-Learning-CVPR16
#include <algorithm>
#include <vector>
...
void LiftedStructSimilaritySoftmaxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  if (this->layer_param_.loss_param().has_weight_source()) {
    ...
    infile.close();
    int dim = this->layer_param_.lifted_struct_sim_softmax_loss_param().dim();
    CHECK_EQ(loss_weights_.size(), dim);
  } else {
    LOG(INFO) << "Weight_Loss file is not provided. Assign all one to it.";
    int dim1 = this->layer_param_.lifted_struct_sim_softmax_loss_param().dim();
  ...
  for (int i=0; i<N_; i++){
    for (int j=i+1; j<N_; j++){
      ...
      if (label_mat[i][j]){
        ...
        Dtype loss_weight_label=loss_weights_[bottom[1]->cpu_data()[i]];
        // squared hinge
        loss += loss_weights_[bottom[1]->cpu_data()[i]]*this_loss * this_loss; 
        num_constraints += loss_weights_[bottom[1]->cpu_data()[i]];
        ...
        for (int k=0; k<N_; k++){
          if (!label_mat[j][k]){
            ...
            scaler = loss_weight_label*dJ_dDjk / sqrt(dot_.cpu_data()[j*N_ + k]);
